...
namespace Com.Latipium.Defaults.Graphics {
	internal class KeyboardHandler {
        private readonly List<Com.Latipium.Core.Tuple<long, Action<int, int>>> Actions;
		...
		private void DispatchEvent(int key, int mods) {
            ...
            foreach ( Com.Latipium.Core.Tuple<long, Action<int, int>> tuple in Actions ) {
		...
		internal void AddHandler(Action<int, int> action, long mask) {
            Actions.Add(new Com.Latipium.Core.Tuple<long, Action<int, int>>(mask, action));
		}
		...
		internal KeyboardHandler() {
            Actions = new List<Com.Latipium.Core.Tuple<long, Action<int, int>>>();
		}
	}
}
